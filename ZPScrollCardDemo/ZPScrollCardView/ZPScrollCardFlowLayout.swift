...
class ZPScrollCardFlowLayout: UICollectionViewFlowLayout {
        ...
        if let attributes = super.layoutAttributesForElements(in: rect),
            ...
            for attri in attris {
                ...
                if isBottomAlign {
                    //下对齐
                    if scrollDirection == .horizontal {
                        let diffY = itemSize.height * (scale - 1) / 2
                        attri.center.y = attri.center.y - diffY + itemSize.height * (self.scale - 1) / 2
                    }
                }
            }
            return attris
